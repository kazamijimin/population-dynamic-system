...
export default function Dashboard() {
  ...
  const stats = [
    { label: "Total Population", value: "12,847", icon: "👥", color: "indigo" },
    { label: "Active Simulations", value: "3", icon: "🔬", color: "emerald" },
    { label: "Inventory Items", value: "156", icon: "📦", color: "amber" },
    { label: "Reports Generated", value: "24", icon: "📊", color: "pink" },
  ];
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto font-sans min-h-screen bg-gray-100">
      {/* Header */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        {stats.map((stat, index) => (
          <div key={index} className="bg-white rounded-xl p-5 flex items-center gap-4 shadow-sm border border-gray-200">
            <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${colorClasses[stat.color].bg}`}>
              <span className="text-2xl">{stat.icon}</span>
            </div>
            <div>
              <p className="text-sm text-gray-500">{stat.label}</p>
              <h3 className={`text-2xl font-bold ${colorClasses[stat.color].text}`}>{stat.value}</h3>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        {/* System Status */}
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
          <h2 className="text-lg font-semibold text-gray-800 mb-5">System Status</h2>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <p className="text-gray-500 text-sm mt-2">Make sure the backend server is running</p>
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              {["Backend API", "Database", "Simulation Engine"].map((service, index) => (
                <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  <span className="w-2.5 h-2.5 rounded-full bg-emerald-500"></span>
                  <span className="text-gray-700">{service}</span>
                  <span className="ml-auto text-emerald-500 text-sm font-medium">Operational</span>
                </div>
              ))}
              {data && (
                <div className="mt-4 p-4 bg-gray-100 rounded-lg">
                  <p className="text-xs text-gray-500 uppercase font-semibold mb-2">API Response:</p>
                  <pre className="text-sm text-gray-700 font-mono">{JSON.stringify(data, null, 2)}</pre>
                </div>
              )}
            </div>
          )}
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {quickActions.map((action, index) => (
            <button
              key={index}
              className={`flex flex-col items-center gap-3 p-6 bg-white border-2 ${colorClasses[action.color].border} rounded-xl cursor-pointer hover:shadow-md transition-all`}
            >
              <span className="text-2xl">{action.icon}</span>
              <span className={`font-medium ${colorClasses[action.color].text}`}>{action.label}</span>
            </button>
          ))}
        </div>
      </div>
